...
import com.wonly.kotlinsample.AppApplication;
...
public final class ResourceUtils {
    ...
    public static boolean copyFileFromAssets(final String assetsFilePath, final String destFilePath) {
        boolean res = true;
        try {
            String[] assets = AppApplication.getInstance().getAssets().list(assetsFilePath);
            if (assets.length > 0) {
                ...
                res = writeFileFromIS(
                        destFilePath,
                        AppApplication.getInstance().getAssets().open(assetsFilePath),
                        false
                );
            }
        } catch (IOException e) {
    ...
    public static String readAssets2String(final String assetsFilePath, final String charsetName) {
        InputStream is;
        try {
            is = AppApplication.getInstance().getAssets().open(assetsFilePath);
        } catch (IOException e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static List<String> readAssets2List(final String assetsPath,
        ...
        try {
            return is2List(AppApplication.getInstance().getResources().getAssets().open(assetsPath), charsetName);
        } catch (IOException e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static boolean copyFileFromRaw(@RawRes final int resId, final String destFilePath) {
        return writeFileFromIS(
                destFilePath,
                AppApplication.getInstance().getResources().openRawResource(resId),
                false
        );
    }
    ...
    public static String readRaw2String(@RawRes final int resId, final String charsetName) {
        InputStream is = AppApplication.getInstance().getResources().openRawResource(resId);
        byte[] bytes = is2Bytes(is);
        if (bytes == null) return null;
        if (isSpace(charsetName)) {
            return new String(bytes);
    ...
    public static List<String> readRaw2List(@RawRes final int resId,
                                            final String charsetName) {
        return is2List(AppApplication.getInstance().getResources().openRawResource(resId), charsetName);
    }
